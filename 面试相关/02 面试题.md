...
- mounted和updated
#### 47.微前端
‌‌‌　　微前端（Micro Frontends）是一种新的前端架构模式，它将一个大型的前端应用拆分成多个小型子应用，每个子应用都可以独立开发、测试、部署和运营。每个子应用都有自己的前端技术栈和独立的代码仓库，可以使用不同的框架（如 React、Vue、Angular 等）来开发子应用。
...
```
- 要指定cookie的父域名，可以将domain属性设置为父域名，例如：
#### 52.Node + js实现[大文件分片上传](https://blog.51cto.com/u_15967457/6170541)基本原理及实践
...
1. 定义用户角色和权限：在系统中定义用户角色和权限，每个角色可以被授予不同的权限，例如可以访问哪些按钮或功能。
2. 根据用户角色确定权限：当用户登录系统时，系统可以根据用户的角色来确定其权限，并相应地显示或隐藏按钮。
3. 隐藏没有权限的按钮：在页面加载时，根据用户的权限来判断哪些按钮需要隐藏。可以使用CSS样式或JavaScript来实现按钮的隐藏。
4. 检查用户是否有权限：在用户点击按钮时，需要检查用户是否具有相应的权限。可以在按钮的点击事件中调用一个方法来检查用户的角色是否具有相应的权限。如果用户角色具有权限，则执行相应的操作，否则提示用户没有权限。
- 总之，要实现全局对按钮的权限设定，需要定义用户角色和权限，并根据用户的角色来确定其权限。在页面加载时，根据用户的权限来隐藏没有权限的按钮。在用户点击按钮时，需要检查用户是否具有相应的权限。这样可以确保用户只能访问其具有权限的按钮，从而提高系统的安全性和可用性。
#### 59.如何保留小数点后两位
...
#### 72.登录和权限是怎么实现的
1. 登录：用户在登录页面输入用户名和密码，前端通过 axios或其他网络请求库向后端发送登录请求，后端根据用户名和密码验证用户身份，并返回一个包含用户信息和 token 的响应。
...
#### 83.history会有个问题，刷新时不请求服务器，怎么解决？
1. 将所有请求都重定向到index.html页面
2. 在index.html中使用JavaScript来根据当前URL加载对应的页面内容
#### 84.清除浮动的三种方式
1. 使用空元素
...
#### 88.了解过IIFE吗？
‌‌‌　　IIFE（Immediately Invoked Function Expression）是一种JavaScript函数的编写方式，它可以立即执行函数并返回结果。
...
如何开启BFC
	...
	○ 行内块元素
#### 94.get和post区别
- GET请求会将请求参数附加在URL的末尾，而POST请求则将请求参数放在请求体中。
- GET请求的请求参数有长度限制，通常为2048个字符，而POST请求的请求参数长度没有限制。
...
#### 98.重绘和回流
重绘：当我们操作的节点上的元素并不导致元素位置发生变化时
回流：当我们的节点发生改变时，浏览器重新渲染部分节点或者全部文档，我们称这个过程为回流
重绘：当元素样式的改变不影响它在文档流中的位置时，浏览器会将新样式绘制到屏幕上，这个过程叫做重绘。重绘不会影响元素的布局和位置，因此它的代价比较小。
回流：当元素的尺寸、布局、内容或者某些属性发生改变时，浏览器需要重新计算元素的何属性（例如位置、大小等），然后再将其绘制到屏幕上，这个过程叫做回流。回流会导其他元素的尺寸、位置等属性发生改变，因此它的代价比较大。
#### 99.sass、less和css的区别
...
- flex=1 表示将元素的 flex 属性设置为 1
- 改变主轴反向的属性是 flex-direction
#### 103.pc端如何做适配?插件需要做哪些配置？具体的配置属性有哪些？按照几倍比例做适配？
...
#### 104.promise说一下？promise.all和promise.race有什么区别？promise的pendding状态可以修改值吗？（更多）
- ###### promise是什么？
...
- ###### promise.all和promise.race有什么区别？
	Promise.all方法提供了并行执行异步操作的能力，并且在所有异步操作执行完后才执行回调.Promise.all来执行，all接收一个数组参数，里面的值最终都算返回Promise对象。这样，三个步操作的并行执行的，等到它们都执行完后才会进到then里面Promise.race 方法与all正好相反谁最先执行完成，就以谁为准执行回调。
...
- 递归也是一种闭包
#### 107.setTimeout是什么任务？nextTick是什么任务？nextclick先执行还是promise先执行？
...
#### 114.登录怎么做的？做过第三方登录吗？如何做登录加密的？
	
#### 115.本地缓存有哪些？有啥子区别？通常什么情况下会使用？
...
- plugins（插件）
#### 117.vue是单页面应用，会导致首屏渲染速率很慢？如何解决？ 与21,22差不多
‌‌‌　　减小入口文件积、UI框架按需加载、图片资源的压缩、组件重复打包、开启GZip压缩
...
- 实现继承的步骤？
	...
	创建父对象、创建子对象并将父对象作为原型对象、在子对象中定义自己的属性和方法，通过子对象访问属性或方法。
...
2. IntersectionObserver是一个api可以实现
	1. 在src目录下建立一个directives的文件夹，在其中建立lazy.js
	2. 在main.js中注册并加载
```javascript
export default {
	mounted(el){
